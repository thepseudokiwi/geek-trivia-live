...
export function BroadcastControls({state,disabled,send}:{state:PrivateLiveState;disabled:boolean;send:(type:LiveCommandType,payload?:Record<string,unknown>)=>void}){
 ...
 return <section className="broadcast-controls">
  <div className="broadcast-heading"><div><p className="eyebrow">Preview / Program</p><h2>Broadcast Controls</h2></div><div><strong>{state.control.audienceCount}</strong> audience client{state.control.audienceCount===1?'':'s'}</div></div>
